                    ...
                    <div class="row w3-res-tb">
                        <div class="col-sm-5 m-b-xs">
                            <span class="btn-group">
                                <a href="{{ route('admin.product.image.index',['id'=>$product->id]) }}" class="btn btn-sm btn-default"><i
                                        class="fa fa-refresh"></i> Làm mới</a>
                                <a href="{{ route('admin.product.image.create',['id'=>$product->id]) }}" class="btn btn-sm btn-success"><i
                                        class="fa fa-plus"></i> Thêm mới</a>
                                <a href="{{ route('admin.product.image.recycle',['id'=>$product->id]) }}" class="btn btn-sm btn-danger"><i
                        ...
                        <div class="col-sm-5">
                            <form enctype="multipart/form-data" method="GET" action="{{ route('admin.product.image.index',['id'=>$product->id]) }}">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li @if ($status == null) class="active" @endif><a
                                                            href="{{ route('admin.product.image.index', ['id' => $product->id,'view' => $view]) }}">Tất cả</a>
                                                    </li>
                                                    <li @if ($status == 1) class="active" @endif><a
                                                            href="{{ route('admin.product.image.index', ['id' => $product->id,'sort_id' => $sort_id, 'status' => '1', 'view' => $view]) }}">Hoạt động</a>
                                                    </li>

                                                    <li @if ($status == 0 && $status != null) class="active" @endif><a
                                                            href="{{ route('admin.product.image.index', ['id' => $product->id,'sort_id' => $sort_id, 'status' => '0', 'view' => $view]) }}">Ngừng hoạt động</a>
                                                    </li>
                                ...
                                <tbody>
                                    @foreach ($productImages as $productImage)
                                        <tr>
                                            <td>
                                                <label class="i-checks m-b-none">
                                                    <input type="checkbox" value="{{ $productImage->id }}"
                                                        name="productImage_id_list[]"><i></i>
                                                </label>
                                            </td>
                                            <td>
                                                {{ $productImage->id }}
                                            ...
                                            <td>
                                                <img style="width: 70px; height : auto"
                                                    src="{{ asset('/storage/images/products/' . $productImage->image) }}"
                                            ...
                                            <td>
                                                @if ($productImage->verified == 0)
                                                    <a onclick="return confirm('Are you sure?')"
                                                        href="{{ route('admin.product.image.verify', ['id' => $product->id,'image_id'=>$productImage->id, 'verified' => 1]) }}"
                                                        class="btn btn-success" title="Activate">
                                                        <span class="glyphicon glyphicon-check"></span>
                                                    </a>
                                                @else
                                                    <a onclick="return confirm('Are you sure?')"
                                                        href="{{ route('admin.product.image.verify', ['id' => $product->id,'image_id'=>$productImage->id, 'verified' => 0]) }}"
                                            ...
                                            <td>
                                                <a class="btn btn-info"
                                                    href="{{ route('admin.product.image.edit', ['id' => $product->id,'image_id'=>$productImage->id]) }}">
                                                    <span class="glyphicon glyphicon-edit"></span>
                                                </a>
                                            </td>
                                            <td>
                                                <a onclick="return confirm('Are you sure?')"
                                                    href="{{ route('admin.product.image.delete', ['id' => $product->id,'image_id'=>$productImage->id]) }}"
                                ...
                                <div class="col-sm-5">
                                    <div class="dropdown">
                                        <button class="btn dropdown-toggle" type="button" data-toggle="dropdown">Hiển thị :
                                            {{ $view }} của {{ $productImages_count }} Hãng
                                            <span class="caret"></span></button>
                                        <ul class="dropdown-menu">
