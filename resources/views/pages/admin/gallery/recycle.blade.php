                    ...
                    <div class="row w3-res-tb">
                        <div class="col-sm-5 m-b-xs">
                            <span class="btn-group">
                                <a href="{{ route('admin.product.image.recycle',['id'=>$product->id]) }}" class="btn btn-sm btn-default"><i
                        ...
                        <div class="col-sm-5">
                            <form enctype="multipart/form-data" method="GET"
                                action="{{ route('admin.product.image.recycle',['id'=>$product->id]) }}">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li @if ($status == null) class="active" @endif><a
                                                            href="{{ route('admin.product.image.recycle', ['id'=>$product->id,'sort_id' => $sort_id, 'view' => $view]) }}">Tất cả</a>
                                                    </li>
                                                    <li @if ($status == 1) class="active" @endif><a
                                                            href="{{ route('admin.product.image.recycle', ['id'=>$product->id,'sort_id' => $sort_id, 'status' => '1', 'view' => $view]) }}">Hoạt động</a>
                                                    </li>

                                                    <li @if ($status == 0 && $status != null) class="active" @endif><a
                                                            href="{{ route('admin.product.image.recycle', ['id'=>$product->id,'sort_id' => $sort_id, 'status' => '0', 'view' => $view]) }}">Ngừng hoạt động</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </th>
                                ...
                                <tbody>
                                    @foreach ($productImages as $productImage)
                                        <tr>
                                            ...
                                            <td><span class="text-ellipsis">{{ $productImage->deleted_at }}</span></td>
                                            <td>
                                            ...
                                            <td>
                                                <a onclick="return confirm('Are you sure?')"
                                                    href="{{ route('admin.product.image.restore', ['id' => $product->id,'image_id'=>$productImage->id, 'verified' => 1]) }}"
                                                    class="btn btn-success"><span class="glyphicon glyphicon-check"></span>
                                                </a>
                                            </td>
                                            <td>
                                                <a onclick="return confirm('Are you sure?')"
                                                    href="{{ route('admin.product.image.destroy', ['id' => $product->id,'image_id'=>$productImage->id, 'verified' => 1]) }}"
